...
export const getIssueTypeLabel = (typeId: string, showHindi: boolean = false): string => {
  ...
  return showHindi && issueType.labelHindi 
    ? `${issueType.label} / ${issueType.labelHindi}` 
    : issueType.label;
};
...
export const getIssueSubTypeLabel = (typeId: string, subTypeId: string, showHindi: boolean = false): string => {
  ...
  return showHindi && subType.labelHindi 
...
/**
 * Validates if a type/subtype combination is valid
 */
export const validateIssueTypeSubType = (typeId: string, subTypeId: string): boolean => {
  const issueType = ISSUE_TYPES.find(type => type.id === typeId);
  if (!issueType) return false;
  
  // Manager type has no subtypes, so any subTypeId is invalid for it
  if (issueType.id === 'manager' && issueType.subTypes.length === 0) {
    return !subTypeId || subTypeId === '';
  }
  
  return issueType.subTypes.some(subType => subType.id === subTypeId);
};
...
/**
 * Gets subtypes for a specific type
 */
...
/**
 * Gets the category of an issue type
 */
export const getIssueTypeCategory = (typeId: string): string => {
  const type = ISSUE_TYPES.find(t => t.id === typeId);
  return type?.category || 'operational';
};
