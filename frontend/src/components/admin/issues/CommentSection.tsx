    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        {issue.comments.length === 0 ? (
          ...
          <div className={`space-y-4 ${!isExpanded && issue.comments.length > 3 ? "max-h-[400px] overflow-y-auto" : ""}`}>
            {(isExpanded ? issue.comments : issue.comments.slice(-3)).map((comment: IssueComment) => (
              <div key={comment.id} className="flex gap-3 items-start bg-gray-50 p-3 rounded-lg">
                <Avatar className="h-8 w-8">
                  <AvatarFallback className="bg-gray-200">
                    {getInitials(commenterNames[comment.employeeUuid] || "UN")}
                  </AvatarFallback>
                </Avatar>
                <div className="flex-1">
                  <div className="flex justify-between items-center mb-1">
                    <span className="font-medium">
                      {commenterNames[comment.employeeUuid] || "Unknown User"}
                    </span>
            ...
            {issue.comments.length > 3 && (
              ...
              >
                {isExpanded ? "Show less comments" : `Show all ${issue.comments.length} comments`}
              </Button>
            )}
          </div>
        )}
