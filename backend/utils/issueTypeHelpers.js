...
// Backend utility to match frontend issue type structure
const ISSUE_TYPES_CONFIG = [
  ...
  },
  {
    ...
    labelHindi: "पर्सनल इन्फॉर्मेशन अपडेट",
    category: 'administrative',
    subTypes: [
...
const getIssueTypeLabel = (typeId, includeHindi = false) => {
  const type = ISSUE_TYPES_CONFIG.find(t => t.id === typeId);
  if (!type) return typeId;
  
  return includeHindi && type.labelHindi 
    ? `${type.label} / ${type.labelHindi}` 
    : type.label;
};

const getIssueSubTypeLabel = (typeId, subTypeId, includeHindi = false) => {
  const type = ISSUE_TYPES_CONFIG.find(t => t.id === typeId);
  ...
  return includeHindi && subType.labelHindi 
    ? `${subType.label} / ${subType.labelHindi}` 
    : subType.label;
};

const getAllIssueTypes = () => ISSUE_TYPES_CONFIG;

const getSubTypesForType = (typeId) => {
  const type = ISSUE_TYPES_CONFIG.find(t => t.id === typeId);
  return type ? type.subTypes : [];
};

const validateIssueType = (typeId, subTypeId) => {
  const type = ISSUE_TYPES_CONFIG.find(t => t.id === typeId);
  if (!type) return false;
  
  if (type.subTypes.length === 0) return true; // Manager type has no subtypes
  
  return type.subTypes.some(st => st.id === subTypeId);
};
