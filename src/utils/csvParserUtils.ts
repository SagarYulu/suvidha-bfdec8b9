  ...
  return new Promise((resolve, reject) => {
    Papa.parse<Record<string, string>>(file, {
      ...
      complete: (results) => {
        ...
        console.log(`[csvParserUtils] CSV parse complete. Found ${results.data.length} rows:`, results);
        
        // Skip processing if no rows were found
        if (results.data.length === 0) {
          console.log("[csvParserUtils] No data rows found in CSV");
        ...
        results.data.forEach((row, index) => {
          ...
          // Properly format dates to YYYY-MM-DD format for database storage
          const dateOfJoining = formatDateToYYYYMMDD(rawDateOfJoining);
          const dateOfBirth = formatDateToYYYYMMDD(rawDateOfBirth);
          
          console.log(`[csvParserUtils] Row ${index} date conversion:`, {
            rawDateOfJoining,
          ...
          // Generate a structured data object for display in the dialog
          ...
          console.log(`[csvParserUtils] Validating row ${index}:`, employeeData);
        ...
        console.log('[csvParserUtils] Validation complete:', {
        ...
        if (validEmployees.length > 0) {
          console.log('[csvParserUtils] Valid employees ready for upload:', validEmployees);
        }
        
        if (invalidRows.length > 0) {
          console.log('[csvParserUtils] Invalid rows:', invalidRows);
        }

        resolve({ validEmployees, invalidRows });
      },
      error: (error) => {
        console.error('[csvParserUtils] CSV parsing error:', error);
        reject(error);
      }
    });
  });
