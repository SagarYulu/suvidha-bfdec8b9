...
interface FeedbackSunburstChartProps {
  data: SunburstItem[];
  title?: string;
  totalCount: number;
}
...
const CustomTooltip = ({ active, payload }: TooltipProps<number, string>) => {
  if (active && payload && payload.length) {
    ...
    // Calculate percentages correctly based on hierarchy
...
const renderActiveShape = (props: any) => {
  ...
  // Only show percentage on segments large enough to display text
  ...
  return (
    <g>
      ...
      {/* Optional: Show line to outside for small segments */}
...
const FeedbackSunburstChart: React.FC<FeedbackSunburstChartProps> = ({ 
  data, 
  ...
  // Generate gradient colors for each segment based on parent color
  const generateGradientShades = (baseColor: string, childCount: number, index: number) => {
    ...
    // Create distinct color variations within the same family
    const hsl = hexToHSL(baseColor);
    
    // Adjust lightness and saturation based on index
    const minLightness = Math.max(hsl.l - 15, 25);
    const maxLightness = Math.min(hsl.l + 10, 70);
    
    const lightness = maxLightness - (index / Math.max(childCount - 1, 1)) * (maxLightness - minLightness);
  ...
  const processData = () => {
    ...
    const innerRingData = data.map((item, index) => {
      // Sort children by value for consistent color assignment
  ...
  const { innerRingData, outerRingData } = processData();
        ...
        <div className="h-[380px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              ...
              {/* Inner ring - Main sentiment categories */}
              <Pie
                data={innerRingData}
                dataKey="value"
                nameKey="name"
...
export default FeedbackSunburstChart;
